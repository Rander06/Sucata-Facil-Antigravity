                    ...
                    {items.map(record => {
                        ...
                        return (
                            <tr key={record.id} className="hover:bg-slate-800/20 transition-all text-xs font-medium">
                                ...
                                <td className={`px-6 py-4 text-right font-black ${record.natureza === 'ENTRADA' ? 'text-brand-success' : 'text-brand-error'}`}>R$ {record.valor.toLocaleString()}</td>
                                ...
                                <td className="px-6 py-4 text-right no-print">
                                    <button
                                        disabled={!activeSession || isLiquidating}
                                        onClick={() => setLiquidationModal({ show: true, record, termId: '', dueDate: record.dueDate || record.due_date || db.getToday(), receivedValue: '' })}
                                        className={`px-5 py-2 rounded-xl font-black uppercase text-[10px] shadow-lg transition-all ${!activeSession ? 'bg-slate-800 text-slate-600' : 'bg-brand-success text-white hover:scale-105'}`}
                                    >
                                        Baixar
                                    </button>
                                </td>
