...
const Dashboard: React.FC = () => {
  ...
  const StatCard = ({ title, value, icon: Icon, trend, trendValue, color }: any) => (
    <div className="enterprise-card p-6 flex flex-col gap-5 bg-slate-900/10 border-slate-800">
      ...
      <div><p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em]">{title}</p><h3 className="text-2xl font-black mt-2 text-white">R$ {value.toLocaleString()}</h3></div>
    </div>
  );
